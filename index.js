...
break
		...
		if (!wokwol.quoted) return reply('Pesan Yang anda reply tidak mengandun  ͬ ͤ ᷮ ᷝ ꙷ  ꙷ ⷶ ͪ  ͩ ͤ ᷜ')
...
break
            ...
            case 'out':
                if (!m.isGroup) return reply(lang.groupOnly())
                if (!m.key.fromMe && !isCreator) return reply(lang.ownerOnly())
               reply('ѕαчσnαrα σníchαn 👋').then(async res => await alpha.groupLeave(from))
            break
...
break
			...
			alpha.sendList(from, 'Mau set menu bang ?', `© ${ownername}`, salam + ' ' +  pushname, 'KLIK DISINI', [
                ...
                if (!listhades.includes(text)) return alpha.sendList(from, 'Mau set menu bang ?', `© ${ownername}`, salam + ' ' + pushname, 'кℓιк ∂ι sιηι', [
							{
								"title": "Set Menu-1",
								"rows": [
									{
...
alpha.sendTextWithMentions(m.chat, teks, m)
await sleep(500)
m.copyNForward(m.chat, true, { readViewOnce: true }).catch(_ => reply('Mungkin dah pernah dibuka bot :3'))
}
...
if (['.next', '.leave', '.вєrhєntí', '.mulαí', 'Cari cαrí tєmαn', 'Keluar', 'lαnjutín', 'Stop'].includes(m.text)) return
